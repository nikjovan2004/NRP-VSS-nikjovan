...
export default function CustomerRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    setLoading(true);
    const result = await register(email, password, name, "customer");
    setLoading(false);
    ...
    <main className="w-full max-w-md space-y-6 rounded-lg bg-white p-8 shadow-md">
      <div className="text-center">
        <h1 className="text-2xl font-bold text-[#1d283a]">
          Registracija za stranke
        </h1>
        <p className="mt-1 text-sm text-[#6b7280]">
          Ustvarite nov račun za naročanje storitev
        </p>
      </div>
      ...
      <p className="text-center text-sm text-[#6b7280]">
        Že imate račun?{" "}
        <Link href="/auth/customer/login" className="font-medium text-[#1d283a] hover:underline">
          Prijava
        </Link>
      </p>
