...
export default function ProviderChatPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!input.trim() || !order || !user || !isMyOrder) return;
    setInput("");
    await sendMessage(order.id, "provider", user.name, input.trim());
  ...
  if (mounted && (!user || user.role !== "provider")) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 px-4">
        <p className="text-[#6b7280]">Za klepet se morate prijaviti kot ponudnik.</p>
  ...
  return (
    <main className="mx-auto flex max-w-2xl flex-col px-4 py-8">
      <Link href={`/provider/orders/${order.id}`} className="text-[#6b7280] hover:text-[#1d283a]">
        ← Nazaj na naročilo
      </Link>
      ...
      <div className="mt-6 rounded-lg border border-[#e5e7eb] bg-white shadow-sm">
        <div className="border-b border-[#e5e7eb] px-4 py-3">
          ...
          <p className="text-sm text-[#6b7280]">Stranka: {order.customerName}</p>
        </div>
